...
//		filterCostCentreCode: [],
		...
		events: function(){
			...
			$(window).resize(function() {
				...
				$(".indentbox").click( function (){				
				var filter = ($(this).attr('id'));
				var flag = ($(this).attr('checked'));						
				textcontent = $(this).next().html();
				switch (filter.substring(0,filter.indexOf('-')).replace(' ','').toLowerCase()) {
					case "jobtitle":
					if(flag == 'true'){
					...
					$(this).css('background-color', '#5D83AA');
					BB.PeopleFinder.filterJobTitle = textcontent.substring(0,textcontent.indexOf(' ('));
					...
					$(this).css('background-color', '#5D83AA');
					BB.PeopleFinder.filterLocation = textcontent.substring(0,textcontent.indexOf(' ('));
					($(this).attr('checked','true'));
					}
					break
					case "contract":
					...
					$(this).css('background-color', '#5D83AA');
					BB.PeopleFinder.filterContract = textcontent.substring(0,textcontent.indexOf(' ('));
					($(this).attr('checked','true'));
					}
					break
					case "sector":
					...
					$(this).css('background-color', '#5D83AA');
					BB.PeopleFinder.filterSector = textcontent.substring(0,textcontent.indexOf(' ('));
					($(this).attr('checked','true'));
					}
					break
				};
		...
		genFilterList: function(config){
			...
			for (rs=0;rs<vSectionList.length;rs++) {
				...
				for (rs2=0;rs2<vOptionList.length;rs2++) {
					...
					hOptionList += '<div id="'+vFilterSection+'-'+rs2+'" class="indentbox" checked="false"></div><div style="display:inline;">'+vName+' ('+vValue+')</div><br/>';					
...
//			var fIncludeOpco = typeof(config.fIncludeOpco) == 'undefined' ? '' : config.fIncludeOpco;
			...
			if (BB.PeopleFinder.resourceUrl == '') { //exit func if true.
			 ...
			 var url = "http://178.23.56.210:8889/vass-peoplefinder/asynchronous";  
        	 var params = {  
...
$(document).ready(function() {
 ...
 var JSON = {
 		rol : '0',
    	action: '',  
        /*Inicializo la parte del script que luego mas tarde rellenare*/  
        script: null,  
        /*Guardo las opciones especificadas*/  
        options: {},  
        /*Llamamos a la url que le pasemos con los paremetros que le indiquemos en la llamada*/ 
        ...
        //Analizo si se debe aniadir el parámetro separado por ? o por &  
        ...
        for(var prop in this.options.params){
            ...
            if (prop='action') this.action = options.params[prop];  
            }
        }  
        var stringParams = params.join("&");  
        //Creo el script o si ya hay uno lo borro
        ...
        //Aniado y cargo el script, indicandole que llame a JSON.process  
        script.src = url + separator + stringParams + (stringParams?"&":"") + this.options.callbackParamName +"=JSON.process";  
       ...
       process: function(data) {
       	var action = this.options.params.action;
        	if (action != 'deleteSearch') {
			 			if (this.action == 'saveSearch') {
