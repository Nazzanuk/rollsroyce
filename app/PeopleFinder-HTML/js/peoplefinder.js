		...
		genFilterList: function(config){
			...
			for (rs=0;rs<vSectionList.length;rs++) {
				...
				for (rs2=0;rs2<vOptionList.length;rs2++) {
					...
					hOptionList += '<li '+vShow+' id="'+vFilterSection.replace(' ','_')+'---'+vName.replace(' ','_')+'"><a href="#">'+vName+' ('+vValue+')</a><span class="BBW-ico-close">&nbsp;</span></li>';
				}
				hOptionList += '</ul>';
				...
				hFilterStartContainer += '<div class="cl"></div>';
            	hFilterStartContainer += '<div id="BBW-wrap-'+vSectionIdentifier+'">';
				
				
				hFilterHead += '<div class="BBW-rbroundbox">';
                hFilterHead += '    <div class="BBW-rbtop"><div></div></div>';
                hFilterHead += '        <div class="BBW-rbcontent">';
                hFilterHead += '            <div class="BBW-wrap-contentmain" id="'+vSectionIdentifier+'" onclick="BB.PeopleFinder.expand(\''+vSectionIdentifier+'\');"><span class="btn-slide">'+vFilterSection+' ('+vOptionListTotal+')</span></div>';
                hFilterHead += '        </div>';
                hFilterHead += '    <div class="BBW-rbbot"><div></div></div>';
                hFilterHead += '</div>';
				
				
				hFilterList += '<div class="BBW-wrap-result" id="BBW-wrap-result-'+vSectionIdentifier+'" style="display:none">';
				hFilterList += '    <div class="BBW-contentresult">';
				hFilterList += hOptionList;
				
				// test how many rows are available to allow show all option
				if (vOptionListTotal>showRows) {
					hFilterList += '        <div class="BBW-showmore ' + vSectionIdentifier + '">';
					hFilterList += '            <a href="javascript:BB.PeopleFinder.showAllFilterOptions(\'' + vSectionIdentifier + '\')">Show all</a>';
					hFilterList += '        </div>';
				}
				hFilterList += '    <input class="filterSearch" name="'+vFilterSection+'" type="text" name="fSearch" />';
			
				//hFilterList += '        <div class="BBW-showmore-input">';
                //hFilterList += '            <input value="Enter '+vFilterSection+'" width="100%" />';
                //hFilterList += '        </div>';
				hFilterList += '    </div>';
				hFilterList += '    <div class="BBW-contentresult-bottom"></div>';
				hFilterList += '</div>';
				
				
				hFilterCloseContainer += '</div>';
				/*xFilter List template*/
				...
				//Append Filter list to existing List or append to empty string if hFilterContainer does not exist
				hFilterContainer += hFilterStartContainer + hFilterHead + hFilterList + hFilterCloseContainer;
			}
			/*xLoop through array*/
			...
			if (vSectionList.length > 0) {
				/******************************************/
				/*Hard code 2 additional filters to append*/
				/*Should create a template method to enable custom values to be parsed instead of duplicating code...*/
				/******************************************/
				var hFilterStartContainer = '';
				var hFilterHead = '';
				var hFilterList = '';
				var hFilterCloseContainer = '';
				var vSectionIdentifier = 'skillsandqualifications';
				var vFilterSection = 'Skills And Qualifications';
				
				/*Filter List template [Skills And Qualifications]*/
				hFilterStartContainer += '<div class="cl"></div>';
				hFilterStartContainer += '<div id="BBW-wrap-' + vSectionIdentifier + '">';
				...
				hFilterHead += '    <div class="BBW-rbtop"><div></div></div>';
				hFilterHead += '        <div class="BBW-rbcontent">';
				hFilterHead += '            <div class="BBW-wrap-contentmain" id="' + vSectionIdentifier + '" onclick="BB.PeopleFinder.expand(\'' + vSectionIdentifier + '\');"><span class="btn-slide">' + vFilterSection + '</span></div>';
				hFilterHead += '        </div>';
				hFilterHead += '    <div class="BBW-rbbot"><div></div></div>';
				hFilterHead += '</div>';
				...
				hFilterList += '<div class="BBW-wrap-result" id="BBW-wrap-result-' + vSectionIdentifier + '" style="display:none">';
				hFilterList += '    <div class="BBW-contentresult"><ul></ul>';
				hFilterList += '    <input class="filterSearch" name="' + vFilterSection + '" type="text" name="fSearch" />';
				hFilterList += '    </div>';
				hFilterList += '    <div class="BBW-contentresult-bottom"></div>';
				hFilterList += '</div>';
				...
				hFilterContainer += hFilterStartContainer + hFilterHead + hFilterList + hFilterCloseContainer;
				/*xFilter List template [Skills And Qualifications]*/
				...
				var hFilterList = '';
				var hFilterCloseContainer = '';
				var vSectionIdentifier = 'experience';
				var vFilterSection = 'Experience';
				
				/*Filter List template  [Experience]*/
				hFilterStartContainer += '<div class="cl"></div>';
				hFilterStartContainer += '<div id="BBW-wrap-' + vSectionIdentifier + '">';
				...
				hFilterContainer += hFilterStartContainer + hFilterHead + hFilterList + hFilterCloseContainer;
			/*xFilter List template  [Experience]*/
			/******************************************/
			/******************************************/
			}
			...
			/*Write filterlist*/
			$('#BBW-filterList').html(hFilterContainer);
...
//					filterCOSTCENTRECODE : BB.PeopleFinder.filterCostCentreCode,
		...
		genRestultList: function(config){
			...
			var $BBWFooter = $('#BBW-footer');
			var $BBWShowmoreTitle = $('.BBW-showmore-title');
			var $BBWWrapSearchResult_BBWHeader = $('#BBW-wrap-search-result .BBW-header');
			//if no results then end function and display no results.
			if (vTotal == 0) {
				$BBWWrapSearchResult_BBWHeader.hide(); //should be set as id
				$BBWFooter.hide(); 
				$BBWShowmoreTitle.hide(); //show filter results text
				$('#BBW-wrap-result').html('<h4>No people could be found.</h4>');
				return;
			} else {
				$BBWShowmoreTitle.show(); //show filter results text
				//display search result footer / header.
				$BBWWrapSearchResult_BBWHeader.show(); //should be set as id
				$BBWFooter.show(); 
			}
			
			
			
			//write out response message
			if (vMessage.length >= 1) {
				$('#BBW-wrap-search-result .BBW-messages').html(vMessage);
				$('#BBW-wrap-search-result .BBW-messages').show();
			}
			...
			/****************/
			var pagingTotal = 10;
			this.TotalPages = Math.ceil(vTotal/pagingTotal); // total result of pages.

			
			var resultFrom = 1;
			var resultTo = pagingTotal;
			if (this.currentPage > 1) {
				resultFrom = ((parseInt(this.currentPage)*pagingTotal) + 1) - (pagingTotal);
				resultTo = (parseInt(this.currentPage)*pagingTotal) - (pagingTotal - response.length);
			}
			
			if (response.length < 10) {
				resultTo = (pagingTotal - (pagingTotal - response.length));
			}
				
			
			$('#BBW-total-currentPage').html(resultFrom); // display current page
			$('#BBW-total-pageList').html(resultTo+'&nbsp;'); // display total results returned.
			$('#BBW-total-results').html(vTotal); // display total results returned.
			var pagingList = ''; //clear paging
						
			//write out 10 paging list
			for (paging = 0; paging < pagingTotal; paging++) {
				//Calculate position of displayed paging list
				if (parseInt(this.currentPage) > 5) {
					var totalPages = vTotal/pagingTotal;
					if (((parseInt(this.currentPage)+paging) - 5) > totalPages) {
						break;
					}
					var pageLabel = ((parseInt(this.currentPage)+1+paging) - 5);
				} else {
					var pageLabel = (paging+1);
				}
				
				//set active page
				if (parseInt(this.currentPage) == pageLabel) {
					active = ' class="BBW-active"';
				} else {
					active = '';
				}
				
				//hide pageset if not available // maybe should use visability style?
				if ((paging+1) > this.TotalPages) {
					active = ' style="display:none;"';
				}
				
				pagingList += '<li'+active+'><a href="#">'+ pageLabel +'</a></li>';
			}
			
			$('#BBW-footer-left').html('<ul>'+pagingList+'</ul>'); //display list of results
			
			//hide previous icon
			var $BBWArrowLeftResult = $('.BBW-arrow-left-result');
			if (parseInt(this.currentPage) <= 1) {
				$BBWArrowLeftResult.hide();
			} else {
				$BBWArrowLeftResult.show();
			}
			
			//hide next icon
			var $BBWArrowrightResult = $('.BBW-arrow-right-result');
			if (parseInt(this.currentPage) >= this.TotalPages) {
				$BBWArrowrightResult.hide();
			} else {
				$BBWArrowrightResult.show();
			}
			/***/
			...
			for (rs=0;rs<response.length;rs++) {
				hResultSet += '<div id="resultSet_'+rs+'" class="BBW-wrap-result-people">'; 
	            hResultSet += '	<div class="BBW-people-image">';
	            hResultSet += '    <img width="75" src="/horizon-public/profile/photo?ProfilePhotoUserGuid='+response[rs].orclGuid+'&ProfilePhotoPictureSize=MEDIUM" border="0" alt="" />';
	            hResultSet += ' </div>';
	            hResultSet += '    <div class="BBW-people-data">';
	            hResultSet += '    	<div class="BBW-wrap-people-data">';
	            hResultSet += '            <div class="BBW-name"><span class="BBW-undescore">'+response[rs].firstName+' '+response[rs].familyName+'</span></div>';
	            hResultSet += '            <div class="BBW-profession">'+response[rs].title+'</div>';
	            hResultSet += '            <div class="BBW-location">'+response[rs].location+'</div>';
				hResultSet += '            <div class="BBW-number">'+response[rs].phoneNumber+'</div>';
	            hResultSet += '            <div class="BBW-number">'+response[rs].mobileNumber+'</div>';
	            hResultSet += '        </div>';
	            //hResultSet += '        <div class="BBW-about-me">';
	            //hResultSet += '        	<div>'+response[rs].about+'</div>';
	            //hResultSet += '        </div>';
	            hResultSet += '    </div>';
				var resultFeature = (parseInt(resultFrom) + rs);
				hResultSet += '    <div>'+ resultFeature +'</div>';
	            //hResultSet += '    <div class="BBW-people-icons">';
	            //hResultSet += '    	<div class="BBW-number">'+response[rs].connections+'</div>';
	            //hResultSet += '    </div>';
	            hResultSet += '</div>';
			}
			...
			/*Write ResultSet*/
			$('#BBW-wrap-result').html(hResultSet);
			/*xWrite ResultSet*/
